...
# Set up logging
logging.basicConfig(level=logging.INFO)  # You can set to DEBUG if you need more detailed logs
...
async def alert_signals(ticker):
    ...
    try:
        # Check if the ticker is a cryptocurrency (you can adjust this to include more cryptos)
        if ticker in ['BTC-USD', 'ETH-USD', 'LTC-USD', 'GC=F', 'SI=F']:  # Correct formats for Yahoo Finance
            data = yf.download(ticker, period='1y', interval='1d')
        else:
            data = yf.download(ticker, period='1y', interval='1d')  # Assume it's a stock ticker
...
# Schedule to run at 6:30 AM PST every weekday (9:30 AM EST)
scheduler.add_job(send_daily_alerts, 'cron', day_of_week='mon-fri', hour=6, minute=30)
...
@bot.slash_command(name='stock', description='Get stock signals for a ticker')
async def stock(ctx, ticker: str):
    ...
    async with ctx.typing():  # Optional: show typing indicator while processing
        ...
        await ctx.respond(signals_message)  # Use respond to reply in a slash command


@bot.slash_command(name="clear", description="Clear messages from the chat")
async def clear(ctx, amount: int):
    """Clears a specified number of messages."""
    if amount <= 0:
        await ctx.respond("Please specify a number greater than 0.")
        return
...
@bot.slash_command(description="Get the stock ticker for a specified company.")
